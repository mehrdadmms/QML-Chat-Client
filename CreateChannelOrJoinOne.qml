...
Page {
    ...
    header: ToolBar {
                Label{
                    padding: 10
                    text: qsTr("Create Channel Or Join One")
                    font.pixelSize: 20
    ...
    function joinFetch(token) {
        ...
        var URL = 'http://api.softserver.org:1104/joinchannel?token=' + token + '&channel_name=' + name;
        ...
        request.onreadystatechange = function(e) {
            ...
            if (request.status === 200) {
                ...
                console.warn('error in creating channel');
                txtJoinError.color = "#c0392b"
    ...
    function createFetch(token) {
        console.log("fetch works create channel ");
        var name = txtInputName.text;
        var title = txtInputTitle.text;
        var URL = 'http://api.softserver.org:1104/createchannel?token=' + token + '&channel_name=' + name + '&channel_title=' + title;
        var request = new XMLHttpRequest();
        request.onreadystatechange = function(e) {
            console.log(JSON.stringify(request));
            if (request.readyState !== 4) {
